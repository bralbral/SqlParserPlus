    ...
    with open(filename, encoding="utf8", errors="replace") as f:
        for s in tqdm(f, desc=f"Parsing {fname}"):
            if lineextractor:
                ...
                em = [
                    email
                    for email in re.findall(regex, s.lower())
...
def tableSelectGUI(tablenames, filename):
    ...
    msg = (
        f"Found {tablecount} tables in {Path(filename).name}. Select Ones You want to"
        " Extract (best looking ones at top)"
    )
    ...
    tablenames.sort()
    upfirst = [
        x for x in tablenames if any(y in x.lower() for y in iwant)
    ]  # put tables with info i like up first
    tot = upfirst + tablenames
    choices = sorted(
        set(tot), key=tot.index
...
def SQLtoJson(filename, ENCODING, FORMAT="json", dumpall=False):
    def find_tables(dump_filename):  # this works
            ...
            for line in tqdm(f, desc=f"Parsing..."):
                ...
                try:
                    if line.startswith("INSERT ") or line.startswith(
                        "insert  into"
                    ):  # took out the lowr as found one sql file where some lines began with insert as name or paswword and added special circumstance for badly dumped dql
                        othertables = "Yup"
                        insertstatement = re.findall(
                            r"\binsert\W+(?:\w+\W+)?into\b", line, re.IGNORECASE
                        )[
                            0
                        ]  # to account for various in insert statements like "insert   into", "INSERT INTO" etc
                        tablename = (
                            line[line.find(insertstatement) + len(insertstatement) :]
                            .strip()
                            .split(" ")[0]
                            .strip(" `\n")
                            .strip('"')
                ...
                if line.lower().startswith("create table"):
                    line = line.replace("IF NOT EXISTS ", "").replace(
                        "&quot;", "`"
                    )  # get rid of those term so below regex works

                    table_name = re.findall(
                        "create table [`']([\w_]+)[`']", line, flags=re.IGNORECASE
                    )
                    if not table_name:
                        table_name = re.findall(
                            "create table ([\w_]+)", line, flags=re.IGNORECASE
                    ...
                    "truncate table"
                ):  # uniqueish case found with "minecraftjunkie.sql" #remove if causing issues elsewhere
        ...
        if len(table_list) > 0:
            print(
                f"Found {Fore.LIGHTBLUE_EX}{str(len(table_list))} {Fore.RESET}tables:"
            )
        table_list.sort()
    ...
    def read_dump(dump_filename, target_table, dumbdump=False):
        ...
        with open(
            dump_filename, "r", encoding=ENCODING, errors="replace"
        ) as f:  # sometimes there are stupid encoding issues where certain char cant be read by encoder, so fuck em
            ...
            for line in tqdm(
                ...
                try:
                    ...
                    if dumbdump:
                        ...
                        if line.lower().startswith("insert") and tableregexp.search(
                            line.split("(", 1)[0]
                        ):  # target_table in line.split("(", 1)[0][:50] and target_table + "_" not in line.split("(",1)[0] and "_"+target_table not in line.split("(",1)[0]:
                            splitline = line.split(target_table)[1]
                            splitfirst = line.split(target_table)[0]
                            if (
                                not splitline
                                or not splitline[0].isdigit()
                                and not splitline[0].isalpha()
                                and (
                                    not splitfirst[-1].isdigit()
                                    and not splitfirst[-1].isalpha()
                                )
                            ):  # take care of when have targetable accounts and dont want values from tables called accounts1 or useraccounts
                                if regexp.search(line):
                    ...
                    if line.lower().startswith("create table") and tableregexp.search(
                        line.split("(", 1)[0]
                    ):  # target_table in line and target_table + "_" not in line.split("(",1)[0] and "_"+target_table not in line.split("(",1)[0]: # the "_" gets rid of false positives like target_table_1 etc but not target_table1
                        splitline = line.split(target_table)[1]
                        splitfirst = line.split(target_table)[0]
                        if (
                            not splitline
                    ...
                    if read_mode == 0:
                        ...
                        if line.lower().startswith(
                            "primary"
                        ) or line.lower().startswith("unique key"):

                            read_mode = 0
                            continue
                        elif line.lower().startswith(
                            ")"
                        ):  # added this to deal with desking, w/o worked fien for other ones
                            read_mode = 0
                            continue
                        ...
                        if line.lower().startswith("insert") and tableregexp.search(
                            line.split("(", 1)[0]
                        ):  # target_table in line.split("(", 1)[0][:50] and target_table + "_" not in line.split("(",1)[0]:
                        ...
                        if (
                            line.lower().startswith("insert")
                            and tableregexp.search(line.split("(", 1)[0])
                            and line.endswith("VALUES")
                        ):
                            pass
                        else:
                            ...
                            if valregex2.search(line):
                                line = (
                                    "(" + re.split(valregex, line, 1)[1]
                                )  # add intro parens back
                            ...
                            if (
                                ...
                                data = re.findall(
                                    "\((.*)\)", line
                                )  # get everythign between first and last occurance of parens
                            else:
                                data = [
                                    line
                                ]  # for times when no parens oustide valies (rare)

                            try:

                                if (
                                    len(data) > 1
                                ):  # for tht etime swhere you have insert values wih all headers before the value sto be inserted, this doesnt matter anymore as getting all data bwn first and last parens
                                    newline = data[1]
                                else:
                                    newline = data[0]
            ...
            if FORMAT == "csv":
                ...
                basepath = os.path.dirname(
                    dump_filename
                )  # dump_filename.rsplit("\\", 1)[0]
                if not os.path.exists(
                    os.path.join(basepath, "SqlConversions", filename)
                ):
                ...
                if len(values) > 0:
                    ...
                    with open(
                        os.path.join(bpath, f"{filename} - {target_table}.csv"),
                        "w",
                        encoding=ENCODING,
                        newline="",
                        errors="replace",
                ...
                if wronglength:
                    with open(
                        os.path.join(
                            bpath, f"{filename} - {target_table}_wrong_length.csv"
                        ),
                        "w",
                        encoding=ENCODING,
                        newline="",
        ...
        if errors:
            with open(
                os.path.join(bpath, f"{filename}_ErroredLines.txt"), "a"
            ) as outfile:
                for x in errors:
                    outfile.write(x + "\n")
    ...
    if not othertables:
        print(
            "Can't find an 'INSERT' statement. Maybe it's not a proper SQL dump. Let me"
            " try something"
        )
        try:
            for x in tables:
            ...
            print(
                "I give up, here's what the first 10 or so lines look like, you tell me"
            )
        ...
        if missing:
            textbox(
                "\n".join(missing),
                "WARNING: These tables that have insert statement but not create table"
                " statement",
                "Right now you'll need to manually extract the table if you want them",
            )
    ...
    if len(tables) == 0:
        print(
            "No create table statements found! Going to try running the 'NoCreateTable'"
        ...
        if not everything:
            ...
            if not everything:
                print(
                    "  Still nothing. Should probably open the file and see what's"
                    " going on"
                )
    else:
        everything = []
...
def tsvtocsv(tsvfile):
    ...
    with open(tsvfile, "r", encoding="utf8", errors="replace") as fin, open(
        os.path.join(bpath, csvfilename), "w", encoding="utf8", newline=""
    ) as fout:
        reader = csv.reader(
            fin, dialect="excel-tab", quoting=csv.QUOTE_MINIMAL
        )  # quote line added because got erro Error: field larger than field limit. some other options for this error: https://stackoverflow.com/questions/15063936/csv-error-field-larger-than-field-limit-131072
        ...
        for line in tqdm(f):
            ...
            if line.strip():  # check if line not empty
                ...
                if tablename:
                    if "VALUES" in line:
                        if (
                            line.split(tablename, maxsplit=1)[-1]
                            .split(maxsplit=1)[0]
                            .startswith("VALUES")
                        ):  # check if just have insert statements without headers
                            vals = getvalues(
                                line, tablename, norepeatinginsert=norepeatinginsert
                            )[0]
                            for x in vals:
                                allvalues.append(x)
                        else:
                        ...
                        try:
                            ...
                            for (
                                x
                            ) in value:  # added these 2 lines with new clean line func
                                x = x.append(tablename)
                                allvalues.append(x)
                        except:
                            print(line)
                            with open(
                                os.path.join(bpath, f"{filename}_Errors.txt"), "a"
                            ) as outfile:
                                outfile.write(line + "\n")
                        if tablename not in tablenames:
                            tablenames.append(tablename)
    ...
    if not isListEmpty(allvalues):
        ...
        if tableheader:
            for x in tableheader:  # create seperate csvs for each table
                ...
                with open(
                    os.path.join(bpath, f"{filename} - {next(iter(x))}.csv"),
                    "w",
                    encoding=ENCODING,
                    newline="",
                ) as f:
                    writer = csv.writer(f)
                    if x[next(iter(x))]:
                        writer.writerow(x[next(iter(x))])
                    for row in flat_list:
                        if len(tableheader) > 1:
                            if row[-1] == next(
                                iter(x)
                            ):  # check if table we want is in value because added tablename to last column of values
                                writer.writerow([z.strip('"') for z in row])
                        else:
                            writer.writerow(
                                [z.strip('"') for z in row]
                            )  # if there's only one table in here, no need to check for table name
        else:
            print(f"    Generating CSV...")
            with open(
                os.path.join(bpath, f"{filename} - tableX.csv"),
                "w",
                encoding=ENCODING,
                newline="",
            ) as f:
...
def getvalues(line, target_table, getheaders=False, norepeatinginsert=False):
    ...
    if getheaders:
        if (
            target_table in line[:50]
            and target_table + "_" not in line.split("(", 1)[0]
        ):
    ...
    elif line.lower().startswith("insert") and tableregexp.search(
        line.split("(", 1)[0]
    ):
...
def cleanline(values, readmode=2, overridequotechar=""):
    ...
    if (
        readmode == 3
    ):  # get raw output of the reader, as otherwise if there is ")" at end os value
        rows = list(reader)
    else:
        for reader_row in reader:
            for column in reader_row:
                ...
                if column[0] == "(":
                    ...
                    if new_row:
                        latest_row = [
                            "" if field == "\x00" else field for field in latest_row
                        ]
...
def getridofuselesscolumns(file):
    ...
    if file.endswith(".csv"):
        try:
            df = pd.read_csv(
                file, encoding="UTF8", dtype=object, index_col=False
            )  # readdirtyfile had issues with tabs etc
    ...
    if all(
        elem in df.columns.values.tolist() for elem in bdaycols
    ):  # check to see if they all in there
    ...
    if all(
        elem in df.columns.values.tolist() for elem in bdaycols2
    ):  # check to see if they all in there
    ...
    df1.to_csv(
        os.path.join(bpath, newfilename), index=False, escapechar="\n"
...
def convertXL2csv(
    directory,
):  # gets Excel file and converts each sheet to CSV and moves XL file to folder
    from tqdm import tqdm
    import os
...
def convertExceltoCSV(filepath):
    ...
    for sheet in sheets:
        ...
        if len(df) > 0:
            df.to_csv(
                os.path.join(directory, f"{filename}_{sheet}.csv"),
                encoding="utf-8",
                index=False,
            )
...
def prettytabletoCSV(filepath):
    ...
    with open(
        filepath.replace(file_extension, ".csv"), "w", newline="", encoding="utf8"
    ) as outcsv:
...
def main():
    ...
    group3 = parser.add_argument_group(
        f"{Fore.CYAN}What Do you Want to convert?{Fore.RESET}"
    )

    group3.add_argument(
        "--sqlextract",
        "-s",
        help="convert SQL file or folder of files to CSV",
        metavar="",
    )
    group3.add_argument(
        "--emailsonly", "-em", help="only extract emails from file", metavar=""
    )
    group3.add_argument(
        "--html", "-html", help="convert file with HTML tables to CSVs", metavar=""
    )
    group3.add_argument(
        "--xltocsv",
        "-xl",
        help="converts each sheet of Excel file to CSV file - throw in file or folder",
        metavar="",
    )
    group3.add_argument(
        "--pretty", "-pt", help="converts 'pretty table' dump to CSV", metavar=""
    )

    group2 = parser.add_argument_group(f"{Fore.CYAN}SQL Dump Options{Fore.RESET}")

    group2.add_argument(
        "--dumpall", "-d", action="store_true", help="grab and convert every table"
    )
    group2.add_argument(
        "--encoding",
        "-e",
        action="store_true",
        help=(
            "add if want to specify encoding or if getting UTF errors. Best not to at"
            " first."
        ),
    )

    group1 = parser.add_argument_group(
        f"{Fore.CYAN}Post Processing Options{Fore.RESET}"
    )

    group1.add_argument("--clean", "-c", help="clean a CSV", metavar="")
    group1.add_argument(
        "--cleandir", "-cd", help="clean a directory of CSVs", metavar=""
    )
    ...
    parser.add_argument(
        ...
        "-r",
        action="store_true",
        help="Use to convert/clean files within subfolders",
    )
            ...
            if args.recursive:
                ...
                for x in files:
                    if (
                        x.endswith(".sql")
                        or x.endswith(".txt")
                        and os.path.isfile(os.path.join(item, x))
                    ):
